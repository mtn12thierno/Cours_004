...
rint((10 > 5) & (6 > 5)) #TRUE car 10>5 est vrai et 6>5 est vrai aussi
...
#3_1
#a: 
rep(c(0,6),3)  #le rep permet de repeter trois le vecteur c qui donne le resultat 
#b: 
seq(from=1,to=10, by=3) *# on parcourt de 1 à 10 en faisant un pas de 3 avec by
#c: 
rep(c(1,2,3),3) #le rep permet de repeter trois le vecteur c qui donne le resultat
#d
x=seq(from=1, to=3) #avec le seq on parcourt de 1 à 3
rep(x,x)   # le rep permet de repeter la valeur prise par x, un nombre de fois = à lui meme
#e
x=seq(from=1, to=3)
y=seq(from=3, to=1)
rep(x,y)
#e
c(2,11,20)/2 # chaque element du vecteur est divisé pa 2 ce qui donne le resultat
#g
rep(seq(from=1, to=3),each = 4) #on parcourt de 1 à 3 et pour chaque pas on repete l'element 4 avec la fonction rep
#3_2
#a
(1:10)/10+1 # on parcourt de 1 à 10 et on divise chaque element par divisé puis on ajoute 1 
#b
vect=1:19
R=vect%%2!=0 # test pour prendre les nombres impairs
print(vect[R])
#c
rep((-2:2), 2)
#d
